...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['qid'])) {
    ...
    if ($page >= $totalQuestions) {
        ...
        mysqli_query($renzo, "INSERT INTO answer (ansid, userId, eid, correct, wrong, date) VALUES ('$ansId', '$studentId', '$eid', '$correct', '$wrong', '$date')");
        unset($_SESSION['displayed_qids'], $_SESSION['correct'], $_SESSION['wrong']);

        echo "<script>alert('Exam completed! Results have been submitted.'); window.location.href = 'tomaraoAvailableTests.php';</script>";
        exit;
    } else {
        $nextPage = $page + 1;
        header("Location: tomaraoQuestionnaire.php?page=$nextPage&eid=$eid");
        exit;
    }
}
