...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['register'])) {
        ...
        $query = "INSERT INTO employee (employee_id, fname, lname, contact, street, brgy, zip, region, province, city, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssssssissss", $employee_id, $fname, $lname, $contact, $street, $brgy, $zip, $region_name, $province_name, $city_name, $status);
        $result = $stmt->execute();
